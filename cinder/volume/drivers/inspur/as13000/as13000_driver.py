    ...
    def initialize_connection(self, volume, connector, **kwargs):
        ...
        if multipath:
            ...
            connection_data = {
                ...
                'target_lun': [int(lun_id)] * len(portal),
    ...
    def _add_lun_to_target(self, target_name, volume):
        ...
        while attempts <3:
            ...
            try:
                ...
                time.sleep(0.1)
                LOG.error('Fail to add lun to target,retrying')
        msg = r'Error running RestAPI : /rest/%s; Error Message: %s, attempts %s' % (
            method, msge, attempts)
        LOG.error(msg)
        raise exception.VolumeDriverException(msg)
